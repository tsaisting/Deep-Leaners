...
```{r}
data_model1 <- model[,c('director','year','duration','votes','budget','reviews_from_users','gender','avg_vote')]  # select some features (no genre)
library(caret)
...
```{r}
lr_model <- glm(avg_vote~ director+year+duration+votes+budget+reviews_from_users+gender,data = data_train, family = "binomial")
