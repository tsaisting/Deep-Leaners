...
## Predictors included 
Seven predictors were selected. Four of them are numerical features, duration, budget, year released, and votes number. The other three are categorical features, movie genre, directors, and actors. For numeric features, we did no change on movie duration and published year. data transformation and normalization were done for budget and votes. 
Budget: In the data set, aside from US dollars, more than 60 other monetary features from other countries existed. We transformed these monetary features into US dollars and logarithmized them. 
Votes: The number of votes contained a large scale and influences the steadiness of the model a lot. We logarithmized votes. 

Three categorical variables needed more work compared with numeric ones. It not only needed reasonable logic but also corresponding programming skills. 
Genre:
Director and Cast:
...
# Model Creation
For this project, our smart questions were to recommend good movies and predict movie ratings. Therefore, we chose the logistic regression model for the first question and the linear regression model for the second question. After finishing the linear regression model, we gained feature importance by building a decision regression tree model.  
...
We built the logistic model with the target variable of the average vote, using 50% of quantile to split the movie average vote and encoding with 1 is a good movie (high average vote) while 0 is a not good movie (low average vote). 
...
Model has AIC value of 8411 and very high residual deviance value of 8376.8.
...
All the coefficients are found significant because of small p-values. Year, director_exp, and the genre of Romance, Horror, Action and Sci-Fi have negative effects on the level of movie rating (avg_vote= 1) while the rest of features have the positive effects on the level of movie rating (avg_vote= 1).
...
We can obtain the value of f1 score, precision and recall score from confusion matrix. Confusion matrix, precision, recall, and F1 score provide better insights into the prediction as compared to accuracy performance metrics. From our confusion matrix, 0.7693 of F1 score is obtained, precision is 0.7715, recall is 0.7671. 
...
The accuracy of train data set is 0.7792 and the accuracy of test data set is 0.7804. 
