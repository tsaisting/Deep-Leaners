...
# voting 
model_data$votes = model_data$votes/1000
...
```{r}
data_model1 <- model[,c('director','year','duration','votes','budget','worlwide_gross_income','reviews_from_users','gender','avg_vote')]  # select some features (no genre)
library(caret)
data(data_model1)
...
```{r}
lr_model <- glm(avg_vote~ director+year+duration+votes+budget+worlwide_gross_income+reviews_from_users+gender,data = data_train, family = "binomial")

lr_model2 <- glm(avg_vote~ year+duration+budget+gender,data = data_train, family = "binomial") # after feature selection 
```
...
```{r bestglm}
data_model1 <- model[,c('director','year','duration','votes','budget','worlwide_gross_income','reviews_from_users','gender','avg_vote')] 
loadPkg("bestglm")
