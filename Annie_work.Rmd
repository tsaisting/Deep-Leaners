...
ggplot(data=data_mg, aes(x=rating, y=count)) + geom_point(col = "seagreen") + labs(title='The amount of attendance of rating vs. movie rating ') + geom_line(col = "seagreen")
...
barplot(colSums(vote[,2:3]), col = c("skyblue3","seagreen"), main="The total votes in female and male")
```
...
colnames(movie_gender) <- c("Genre","All gender","Male", "Female")
barplot(colMeans(movie_gender[,3:4]), col = c("skyblue3","seagreen"), main="The average of movie rating in female and male")
```

```{}
# plot for all rating of gender together
#melt data frame into long
colnames(movie_gender) <- c("genre","all gender","male", "female")
# <- melt(movie_gender, id.vars = 'genre', variable.name = 'gender')
#movie_gender <- subset(movie_gender, value >= 8)
#ggplot(data=movie_gender, aes(x=reorder(genre, value), y=value, fill = gender)) + geom_bar(stat = "identity", position="dodge",alpha=.7) + scale_fill_brewer(palette = "Set1") + xlab("movie genre") + ylab("Rating")+ labs(title="Rating plot with different genders(rating >= 8)") + coord_flip() + theme(legend.position="top")
