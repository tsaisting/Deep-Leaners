...
Prior to 2020, watching movies has become a popular way of mass leisure. With the rapid development of the Internet, film review aggregators such as IMDb and Rotten Tomatoes have gradually popularization. The film reviews have a great reference significance for the majority of ordinary movie audiences and also for the film production company. As a general rule, movies with a higher rating score would obtain more attention and then get a higher paying off at the box office. However, the movie reviewers who are hired by stakeholders, pretend to be ordinary netizens, giving high movie ratings, and posting favorable comments on the website to make profits. In addition, some of the netizens give extreme movie ratings based on the personal likes and dislikes of a certain director or actor, instead of standing in neutrality. The bogus movie ratings would mislead the public and might cause adverse effects. The issue of bogus movie comments has aroused public attention. Many researchers have also done a lot of work in the field of bogus comment detection. At present, most of the work focuses on judging whether a specific user or comment is reliable. In this project, the team is taking an approach to construct a rating system that analysis of the average vote of the movies, since it is the main factor for users to judge the quality of the movie as well. Starting with the overall distribution of movie ratings, the influence of bogus reviews on the average movie score would be eliminated and a movie rating distribution with authenticity would be established. The team would also explore what factors that have an effect on the movie ratings and the interaction effect between factors to build a prediction model that is built to help the clients to understand the trends in the film market so that the client can make the right decisions on movie production.

## Prior Researches
For IMDB movies reviews, one prior research has been done with the movie review for sentiment analysis in text classification. A dataset comprised of 1,000 positive and 1,000 negative movie reviews was analyzed and used in the research on natural language processing. (Brownlee, 2020)

For movie review classification, bag-of-words feature extraction technique is used to extract unigrams, bigrams, and trigrams as a feature set from given review documents and represent the review documents as a vector. The Na¨ıve Bayes algorithm is employed to categorize the movie reviews into negative and positive reviews. For the task of movie review summarization, word2vec model is used to extract features from classified movie review sentences, and then semantic clustering technique is used to cluster semantically related review sentences. Different text features are employed to compute the salience score of all review sentences in clusters. (Khan etc., 2020)
...
The dataset is from the kaggle website,divided into three separate csv files .It contains 22 variables of 85,855 movies spanning over 1894 to 2020 January over multiple countries. Further, contains 297,705 cast instances corresponding to the movies. The "in development" titles are not included in the files and moreover contains  missing categories of data like: the short plot on the main page, awards, external reviews, parents guide, synopsis, faqs, news etc .Additional features as production company , title groups, adult titles, instant watch options like prime, Netflix if provided could make the analysis much broader and ideal.
...
# SMART Questions
The data set of IMDb Film features was gotten from Kaggle. There are 21 features, such as movie genre, director, released year, actors, duration, movie language, rating, vote numbers, vote demographics, etc. The aim of this project is to explore the factors that affect movie ratings and the influence of different ages on the choice of movie genre. Six independent variables, the difference of Movie genre, the choice of director, the mean age of actors, the movie language, the voting demographic(gender and age), and the age of viewers are taken into account. These variables represent different aspects of the influence on movie ratings. Four of them focus on the construction of movies. For example, Movie genre, the choice of director, the mean age of actors, and the movie language. Other variables focus on the character of voters. Such as, the age and gender of voters.

At first, basic EDA was done by selecting quantitative variables, plotting movie rating histogram and bar graphs. During the EDA, one quantitative variable, movie duration, seemed to could be taken into account. The relationship between movie duration and ratings could be analyzed. Therefore, the previous fourth question about the mean age of actors was switched to this one.
